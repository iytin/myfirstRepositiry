...
<? require_once 'conectdb.php';?>
 ...
 <script type="text/javascript">
	 var myMap,gg=0,open=null,bopen=null;var eventsGroup=null,  mapSize=null,zoom,openForZoom=null, mov=null; 
ymaps.ready(init);// Как только будет загружен API и готов DOM, выполняем инициализацию 
  function init () {
    ...
    maxWidth: 1000,//максимальная ширина
    minWidth: 1000,//минимальная ширина(они должны совпадать)
    minHeight: 330,
    start: function(event, ui) {//действия которые выполняются при старте изменения размеров карты
        ...
        /** скрываем форму ссылки на карту  */
    ...
    resize: function(event, ui) { //функия действий которые выполняются при непосредственно изменении размеров карты
            ...
            myMap.balloon.close();//закрываем балун при изменении размеров карты
            mapSize = myMap.container.getSize();//при движении заполняем картой всю нужную область
    } 
    }); 

    /// создаю шаблон для балуна метки
    var myBalloonContentBodyLayout = ymaps.templateLayoutFactory.createClass(
    '<div>$[properties.body]<p><div>$[properties.metro]</div></div>' 
    );
     //* конец описания шаблона метки*/ 
     ///****
     ///****
     ///создание шаблона для кластерного балуна
     MyMainContentLayout = ymaps.templateLayoutFactory.createClass('', {
                        build: function () {//функции которые выполняются при создании балуна
                            ...
                            bopen=this.activeObject.properties.get('myid');//записали ИД открытого элемента
                            this.applyContent();
                        },
                        clear: function () {
                        ...
                        onStateChange: function () {
                            ...
                            if (newActiveObject != this.activeObject) {
                                ...
                                bopen=newActiveObject.properties.get('myid');  //записали ИД нового открытого элемента                              
                                this.applyContent();
                            }
                        },
   ...
   var cluster = new ymaps.Clusterer();//создаем кластер меток
    cluster.options.set({
        clusterBalloonMainContentLayout: MyMainContentLayout,//указываем какой шаблон использовать (он описан выше)
        gridSize:50,//размер кластерной сетки
        clusterBalloonHeight:320,//высота балуна
        clusterBalloonSidebarWidth:25,//ширина правой стороны балуна (список меток)
        clusterBalloonPane: 'movableOuters'//свойство выхода балуна за пределы карты
    });
              ...
              $('#help-visibility').click();
                ...
                if (geoObjectState.isShown) {
                    ...
                    // Если объект попадает в кластер, открываем балун кластера с нужным выбранным объектом.
                        ...
                        geoObjectState.cluster.state.set('activeObject', myPlacemark[PLBC]);//делаем активным именно этот элемент
                        PLBC='';
                }
          }
    });
cluster.events.add('balloonclose', function (e) {bopen=null}) //событие закрытия балуна

cluster.events.add('balloonopen', function (e) {//события по открытию кластера
 ///// создание смещения метки к низу карты (смещает вниз и по центру)
        ...
        var geo=cluster.geometry.getCoordinates(),
        ...
        myMap.panTo(projection.fromGlobalPixels([globalPixel[0], (globalPixel[1] - size[1]/2 +10) ], myMap.getZoom()),
                    {delay: 0});
    //////////////////////////
 var names=[],carr=[];var oldindex='';
               
                    var coords=cluster.geometry.getCoordinates()
                    ymaps.geocode([coords[0],coords[1]],{kind:'metro', results:4,spn:[0.01,0.01]}) //ищем метро
                            .then(function (res) {                                
                                ...
                                var mrt=[],g=1,str='';
                                names.forEach(function(index2,idc){
                                    if(g>2){

                                    }else{
                                        if(index2 && index2!=oldindex){
                                        oldindex=index2;
                                            if(ymaps.coordSystem.geo.getDistance(carr[idc],coords)<=1000){
                                            str+=' '+index2+'('+Math.round(ymaps.coordSystem.geo.getDistance(carr[idc],coords))+' м.)';
    ...
    if(geoObjects){
        geoObjects.forEach(function(index, i){   //перебираем все элементы которые остались в кластере при максимальном зуме     
            (function (currentGeoObject) {
                currentGeoObject.properties.set({
                    clusterCaption: i+1//создаю пагинатор
                });               
                                if(currentGeoObject.properties.get('change')==0){  ///если еще не были загружены данные для метки
                                $.post("obrabotka.php",{idget:currentGeoObject.properties.get('myid')},function (json){//грузим данные с базы
                                                var strbal=json.markers[0].f_n+'<p>'+
                                                    ...
                                                    currentGeoObject.properties.set({
                                                        balloonContentBody: strbal+str,//устанавливаем полученые значения в текущую метку
                                                        change:1//изменено
                                                    });
                                        },'json');
                        }
    ...
    function work() {
            if ($(".open").css('height')=="0px"){
                ...
                function() {                
                    ...
                    myMap.container.fitToViewport();//растягиваем карту по всей ширине
                    mapSize = myMap.container.getSize();//узнаем размеры карты
        ...
        myMap.setZoom = function (zoom, options) { //переопределении зума карты (будет нужным при доработке функционала)       
            ...
            if (self.balloon && self.balloon.isOpen()) {
                var balloonPixel = myMap.options.get('projection').toGlobalPixels(self.balloon.getPosition(), this.getZoom()),
                    offsetBase = [(balloonPixel[0] - globalPixelCenter[0]), (balloonPixel[1] - globalPixelCenter[1])];
 ...
 */       
    myMap.balloon.events
                // При открытии балуна начинаем слушать изменение центра карты
                    .add('open', function (e) {//при открытии балуна
                     var balloon = e.get('balloon'),
                         isRemoved = false;
                        eventsGroup = myMap.action.events.group().add('tick', function (e) {//слушаем передвижение карты                            
                        ...
                        if(balloon.getData().properties.get('myid')){
                            openForZoom=balloon.getData().properties.get('myid');//значение ИД для постоянно открытого алуна
                        }
                    })
        ...
        function isPointInBounds (mbr, point) {//проверяет вышел балун за карту или нет
        ...
        myMap.balloon.close();///закрываем балун при сворачивании карты
                $(".open").animate({
                    "height": "0px"},
                function() {
    ...
    function razvert(){
                ...
                button = new ymaps.control.Button({
                    data : {
                        content : '<img src="img/0.png" style="margin-top:6px;"/>',//устанавливаем картинку кнопки
                        title : 'Поиск местонахождения'
                    }
                });
            // Если кнопка активна, то карта разворачивается во весь экран, иначе - 600x400.
         button.events
         .add('select', function (e) {//событие нажатия кнопки поиска местонахождения
         $('#help-visibility').click();
            if(button.isSelected()) {
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                    function (position) {
                        createGeoI(position.coords.latitude,position.coords.longitude); //создаем метку на карте
                         myMap.zoomRange.get(position.coords.latitude,position.coords.longitude).then(function (range) {//функция для поиска существующего зума
                                if (myMap.getZoom() > range[1] ) {//изменяем зум
                                    myMap.setZoom(range[1]); 
                                } 
                            });
                    });
        ...
        .add('deselect',function(e){//при отжатии кнопки удаляем метку
         myMap.geoObjects.remove(myPl);
        });
            // Добавление панели инструментов на карту
            myMap.controls.add(button, { top : 5, left : 98 });//смещаем эту кнопку немного в сторону
   }
   ...
   iflag=[lat,longs];
		...
		myMap.geoObjects.add(myPl);  
                myMap.zoomRange.get([lat,longs]).then(
                    function (range) {//функция для поиска существующего зума но не больше 15
                    range[1]=15;
                    if(range[1]>=15){
    ...
    function getdata(datas){//бработка данных с бзы
       var json;
       $.post("obrabotka.php",{query:datas},function(json){
            ...
            for(var j=1; j<=myCollection.length; j++){
                if(myCollection[j]){
            ...
            myMap.geoObjects.add(cluster);//добавление кластера на карту
  ...
  function createMetka(id,x,y,id_s){//функция создания метки и всей параметров для нее
        ...
        coord.push(x);
        coord.push(y);
        if(myCollection[id_s]==null){// если нет еще такого массива меток то создаем его
            myCollection[id_s] =[];  
        }
        myPlacemark[id] = new ymaps.Placemark(coord,///создается метка со всеми параметрами
        {   
            myid:id,//ИД метки
            body : 'Загрузка...',//значение по умолчанию
            change:0//флажек о изменениях
        }, 
        {
              balloonPane: 'movableOuters',//выход метки за карту
              balloonMinWidth:500,//миним ширина
              balloonMinHeight:300,//миним высота
              balloonAutoPanMargin:[100,250],//сдвиг балуна
              balloonContentBodyLayout:myBalloonContentBodyLayout,//шаблон балуна (описан выше)
              balloonAutoPan:true,
              preset: setStyle(id_s-1)//стиль метки
        }); 
        myCollection[id_s].push(myPlacemark[id]);//добавляем в массив меток
        myPlacemark[id].events.add('balloonopen', loadContentBalloon);//событие при открытии метки
        myPlacemark[id].events.add('balloonclose', function(){open=null;});//при закрытии метки
  }
function loadContentBalloon(e){//функция открытия метки
    ...
    /////*****
        ...
        var name=e.get('target').properties.get('myid');//Ид метки
        open=name;
            var names = [],str='', carr=[];
            var coords=e.get('target').geometry.getCoordinates();//координаты метки
            var oldindex='';
                    if(name!=0){
                        if(e.get('target').properties.get('change')==0){/// если не было изменений
                            ymaps.geocode([coords[0],coords[1]],{kind:'metro', results:4,spn:[0.01,0.01]}) //ищем метро
                            .then(function (res) {
                                res.geoObjects.each(function (obj) {
                                    names.push(obj.properties.get('name'));
                                ...
                                var mrt=[],g=1;
                                names.forEach(function(index,idc){//перебираем найденые метро на ссответствие растояния
                                    if(g>2){}else{
                                        if(index!='' && index!=oldindex){//если такое метро еще не записано
                                        oldindex=index;
                                            if(ymaps.coordSystem.geo.getDistance(carr[idc],coords)<=1000){
                              ...
                              if(e.get('target').properties.get('change')==0){//если нет изменений
                                 e.get('target').properties.set('change',1);//ставим что изменено
                                    $.post("obrabotka.php",{idget:name},function (json){
                                        ...
                                        e.get('target').properties.set('change',1);//ставим что изменено
                                        e.get('target').properties.set('balloonContentBody','');//установили точки метро 
                                    },'json'); 
                                }
                            }); 
                            }
    ...
    function searchRegion(){//функция поиска региона
        var region=$("#region").val();//название региона
        if(region!=''){
                $('.load_block').css('display','block');
                ymaps.geocode(region,{results:1})//поиск границ региона
            .then(function (res) {
                ...
                var BoundX=Bound[0]; 
                iflagR=1;
                myMap.panTo([Number(pos[0]),Number(pos[1])], {flying:true});//пеемещаемся к найденому региону
                    myMap.setBounds([
                        BoundX,
                        BoundY,
                    ]);
    ...
    function createMetkaRes(id,x,y,id_s){//функция создания меток РЕЗУЛЬТАТА поиска (теже параметры что и обычной метки см. выше)
       var coord=new Array();      
       coord.push(x);
       coord.push(y);
   ...
   function setLocationParam () { 
       ...
       if(masrem[0]!=''){
        ...
        if(bopen){
           ...
           var myurl='http://svadbagolik.cybers.net.ua/indexnorm.php?'+encodeURI(params.join('&')); //собственно, сама ссылка в сборе
            $('#link').val(myurl);
        }    
